...
public class Permisos extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //se brinda o pregunta si tiene LOS permisoS
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_permisos);
        if (ContextCompat.checkSelfPermission(this,
                Manifest.permission.CALL_PHONE) == PackageManager.PERMISSION_GRANTED) {//se brinda o pregunta si tiene el permiso
            arranque();//si tiene ejecuta el metodo arranque
        } else {//si no tiene el permiso muestra ese texto de abajo
            /*
            solicitarPermiso(Manifest.permission.CALL_PHONE,
                    "Sin el permiso" + " de telefono no podemos realizar llamadas.", 0);//este codigo 0, puede ser cualquier numero


            Handler handler = new Handler();
            handler.postDelayed(//retraso de 2 segundos
                    new Runnable() {
                        public void run() {
                            solicitarPermiso(Manifest.permission.CALL_PHONE,
                                    "Sin el permiso" + " de telefono no podemos realizar llamadas.", 0);
    ...
    */
    //ESTE  METODO PIDE EL CHECK
    public void solicitarPermiso(final String permiso, String justificacion, final int codigo) {
        if (ActivityCompat.shouldShowRequestPermissionRationale(this, permiso)) {
            AlertDialog.Builder dialogo1 = new AlertDialog.Builder(this);//dialogo del check
            dialogo1.setTitle("Solicitud de permiso");
            dialogo1.setMessage(justificacion);
            dialogo1.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialogo1, int id) {
                    ActivityCompat.requestPermissions(Permisos.this, new String[]{permiso}, codigo);//aparece el dialogo - no se puede personalizar nada
                }
            });
            dialogo1.show();
        } else {
            ActivityCompat.requestPermissions(this, new String[]{permiso}, codigo);// SIN EL CHECK - EN LA 1ERA VEZ(ESTE NO SE PUEDE PERSONALIZAR)
        }
    }
}